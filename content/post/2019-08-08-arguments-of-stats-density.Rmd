...
One of the "go to" functions for [kernel density estimation](https://en.wikipedia.org/wiki/Kernel_density_estimation) is [density()](https://rdrr.io/r/stats/density.html) from base R package 'stats'. Given numeric sample, it returns a set of x-y pairs on estimated density curve. It is also a main "workhorse" for estimating continuous distributions in my ['pdqr'](https://echasnovski.github.io/pdqr) package.
...
So in this post I illustrate with animations how changing arguments of `density()` changes the output.

# Overview

The illustration workflow will be as follows:

- For a certain `density()` argument create a set of its values that will help illustrating how it affects the output.
...
density_anim <- function(..., duration = 10) {
  ...
  if (is.list(vals)) {
    ...
    # Converting to factor to preserve order of argument values in animation
  ...
  anim <- tibble(!!sym(arg) := states) %>% 
    ...
    ggplot(aes(x, y, group = !!sym(arg))) +
      ...
      # Curves for different argument values
      geom_line(size = 1.5) +
...
Argument `weights` should be used if some observations are considered to be "more important" and should be used more as a reference than the other ones. It should be a numeric vector with the same length as input sample `x`. **Note** that for output to be a true density plot, sum of `weights` should be 1.
...
Arguments that define structure of output are: `n`, `from`, `to`, and `cut`. There are also `give.Rkern` (can be used to return computed "canonical bandwidth") and `na.rm` (whether to remove `NA` values from input), which are not of particular interest here.
...
Note that when `from` is equal to the smallest value of input numeric vector, it is still greater than default left edge of density curve. For explanation of this "extending property" of default curve, see section about `cut`.
...
When `cut` changes from 3 to 0, range of computed points shrinks from default one to range of input numeric vector. The fact that by default `density()` computes set of points outside of original range I call its "extending property". **Note** that `cut` can also be negative, which means reduced input range.
