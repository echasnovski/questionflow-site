...
This announcement post will be in form of a short problem solving cookbook with a little intro to package core structure. For more details go to these resources:
...
The main structure unit of the package is **pdqr-function**: an actual function which represents distribution with finite support and finite values of probability/density. Distributions with infinite support and/or density can be approximated.
...
For continuous type, [density()](https://rdrr.io/r/stats/density.html) function will be used to estimate distribution's density (if sample has 2 or more elements). **Note** that arguments of `density()` will affect greatly the output distribution (as always in kernel density estimation).
...
These newly created functions can be used to obtain probability or density for any numbers:
...
**Solution** R-functions are used for generating samples. Currently there are two ways of creating them:
...
**Solution** First, you need to create pdqr-function for random variables. Here standard normal distribution will be used. This can be done by converting base R distribution functions to be pdqr-functions. After that, using `+` gives distribution of sum of two random variables:
...
- Distribution of sum of random variables is computed with simulation: big samples are drawn from both distributions, their sum is computed (resulting in a sample from sum of random variables), and target distribution is created using one of `new_*()` functions.
...
mix_weights <- c(0.2, 0.8)
(d_norm_mix <- form_mix(d_norm_list, weights = mix_weights))
...
- Estimate sample distribution. It can have any type, depending on problem. To emulate traditional sample estimates, "discrete" type is better suited.
...
```{r mode}
mtcars$mpg %>% 
  # Using "discrete" type will compute the most frequent element of sample
...
# Compute region width (sum of width of all disjoint intervals)
...
**Problem** Compute a probability that random draw from one distribution is greater that random draw from another distribution.
...
- Consider using this package for solving your practical problems. Please feel free to send me your feedback, ideas, and suggestions.
